...
Hong Kong and Shanghai both run `1.0.4-9ca10b44507a` with GameState v36, cloud schema v7 and SQLite layout v2; both code rollback targets are `1.0.3-6d59252f4f15`. Main-slot upload, automatic sync and history restore update the leaderboard from server-derived save metrics; startup backfill is idempotent, manual slots do not participate, and accounts can opt out. Production checks confirmed gzip for hashed JS/CSS, immutable asset caching, no-cache HTML/service-worker behavior, active services, `NRestarts=0` and no DSP 5xx in the latest 500 requests. Hong Kong also authorizes the packaged Android `https://localhost` origin while rejecting unknown origins. Layout v2 keeps cloud-save payloads in independent revision rows, and code rollback never restores the database. Brotli remains optional. Shanghai is about 92% full with roughly 4.6 GiB free, so release directories, native binaries, logs and backup retention need close monitoring.
