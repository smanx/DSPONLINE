...
test("English light release notes are localized and persist dismissal", async ({ page }) => {
  ...
  await page.goto("/?menu=1&lang=en");
  const dialog = page.getByRole("dialog", { name: "Cloud Reliability, Rankings, and Cross-platform Experience" });
  await expect(dialog).toBeVisible();
  await expect(dialog).toContainText("1.0.40");
  await expect(dialog).toContainText("Your real Galaxy rank is visible");
