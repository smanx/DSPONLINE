...
香港、上海 Web/API 与上海下载站均已切换到 `1.0.24-019bac527829` / GameState v46。两地继续使用云 schema v7 和 SQLite layout v2，代码回滚不得恢复数据库；香港 `/downloads/*` 仍重定向上海。1.0.24 Android SHA-256 为 `e5761b3b9cab5b635c3c2d9d2f5a86c4b86c5c4ec6d54c0a03599b6c82da455a`，Windows SHA-256 为 `c3b8e68abffe68a2dfd6627c128ed6a0cf77065a05dcc9210bb533800ab8e3b0`（Authenticode `NotSigned` 测试包）。两地发布前 Backup API 快照均通过 `quick_check`，Web/API 和下载回滚目标均为 `1.0.23-867e5101b689`；公网健康、下载页、完整下载哈希、Range、缓存头和上一版 hashed chunk 回退均通过。两地应用 Nginx 规则现使用 40 MiB 上传边界，固定切换脚本会把当前和目标 release 的 hashed assets 归档到 `/var/www/dsp-idle/shared/assets`。香港发布后在线手工备份曾因活跃写入下高 I/O 不收敛而安全中止，不完整目标已删除，发布前有效备份保留；在完成低流量诊断前不要在生产高峰直接重跑。完整证据见 [releases/1.0.24.md](./releases/1.0.24.md)。
...
Brotli 仍是可选后续项，应先用真实流量比较 CPU、缓存命中和传输节省。不要用“提高服务器配置”替代静态压缩、缓存和 chunk 体积治理；当前 2 核 2 GB 对首版 Node + Nginx + SQLite 足够。1.0.24 发布和临时上传清理后，上海与香港根盘均约剩 13 GiB；历史 SQLite 备份与旧发布目录仍保留，继续按备份保留/异地归档告警运营，且清理不得删除当前版、回滚版或有效备份。
