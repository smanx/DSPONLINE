...
> **当前发布基线（2026-08-02）**：当前物流重构使用 `1.0.21 / GameState v46`；有效资产的 v43 空间站实验存档拒绝加载。量子网络回归以 GameState v46 语义为基线，并保留传统物流站升级兼容测试。
...
| 单元/领域 | `npm test` | `1.0.21`：74 个文件通过、1 个跳过；651 项通过、9 项跳过 | 引擎暂停边界、量子共享带宽、直接入库、云存档兼容、轨道采集接入、本地运输兼容、v1-v46 存档、超大蓝图、递归副产物、异步确认框、统计窗口、确定性物流、离线 Worker、内容包注册表同步和云同步等 |
| 浏览器 E2E | `npm run test:e2e` | `1.0.21` 聚焦 27/27；全量复跑本次超时，不计为通过 | 版本公告、开局到银河终局、量子库存、内容包 Worker、待建施工、蓝图往返、双语亮暗主题、新旧手机壳、390×844 与 200% 字号、暂停画布、IndexedDB 存档保护和横竖屏回归 |
| 云服务 | `npm run test:server` | `1.0.21`：46/46 | 用户名注册、四槽云存档、schema v3→v7、SQLite layout v1→v2、v35-v46 存档边界、旧 quantumTarget 兼容、完整性/格式/体积错误、无人工封顶与极值饱和排名、内部排行榜限制和管理员保护 |
...
| 生产构建 | `npm run build` | 1,866 个模块 | `tsc -b`、Vite chunk、Worker 和 PWA 资源 |
