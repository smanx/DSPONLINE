...
| 单元/领域 | `npm test` | 工作区 268 项；线上基线 254 项 | 引擎、v1-v30 存档、存档配额急救、外星系原油幂等迁移、采集设备回收、递归灰锤、殖民条件、四向分流、托盘上限、科研、电力、蓝图、规划、网络、性能与分槽云同步等 |
| 浏览器 E2E | `npm run test:e2e` | 工作区 97 项；线上基线 93 项 | 从开局到银河终局、保存失败急救、区域触摸缩放、灰锤单击、巨构真实尺寸、科技树滚轮隔离、右键菜单策略、双指从节点接管、星图条件、200% 字体和桌面/手机横竖屏回归 |
...
## 11. `0.5.0` / v30 候选版本地验收（未发布）

本节描述 2026-07-23 当前工作区；香港与上海仍运行 `0.4.0` / v28。
...
| `npm run test:ops` | 5/5 通过 |
| `npm run build` | 通过 |
| `npm run test:e2e` | 97/97 通过，串行约 5.1 分钟 |
...
尚未完成的发布门禁仍是 Android Chrome 与 iPhone Safari 各 30 分钟真机温度、耗电和帧率测试；本候选版尚未部署、未创建发布标签。
