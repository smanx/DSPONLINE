...
> **1.0.46 完整开发门禁（2026-08-19，未发布）**：固定运行时源码 `1c01e79384283f02057e8c56d30b497fa3c5842e`。普通构建默认采用 1.0.43-compatible verified-primary 协调器，durable runtime/WAL 只有显式 `VITE_DURABLE_RUNTIME_RECOVERY=true` 才启用；空间站 v46 bridge 强制稳定路径。durable 故障 E2E 7/7 覆盖 finalize、二次 persistence、T1/head rollover、运行中自动保存恢复、实验性编辑保存失败、pagehide 与撤销/视口/activity 回放。两份真实玩家档各完成两次 30 秒自动保存，均保持 Worker active、`paused=false`，重载后主档运行且源文件 SHA-256 不变。手机连续拉线覆盖重复/非法点击非阻塞、最终原子失败零创建零扣料、6/10/50/100 候选、390×844/360×640/844×390 与 80%～200% 字体；dynamic-import 分类不再吞掉普通 `TypeError`。画布门禁覆盖网络聚焦下交互目标不透明、LOD 展开无隐藏帧、dim 节点可平移画布、一行配方/产物标签、重叠标记与“完整 + 全部卡片”480/1,000 密集保护。全量 Vitest 1,422/20；server 363/2 + station 3/3；ops 56/6；release-switch 29/29；native 24/24；licenses 125 包；匿名 v47 发布合同 12/12；Chromium 422/24；production preview 26/26；Firefox/WebKit 2/2；两份真实存档各 18 张画布矩阵且 pageerror 0；根/server audit 0 漏洞。`npm run build:web` 为 1,961 modules，startup 194,823 B gzip、menu 281,392 B gzip、forbidden startup modules 0，Build ID `1.0.46+1c01e7938428`。release gate 强制 Web 平台构建并在其后运行 PWA 生命周期，禁止复用 Android/Desktop `dist`。未签名 Windows/Android 诊断制品已构建；正式签名、实体设备与线上发布仍未执行。

> **1.0.46 画布展示增量门禁（2026-08-19）**：一行卡及 React Flow wrapper 固定 `96×32`；数量标记为 `88×44` 点击区和约 `80×30` 可见胶囊；大幅平移后 `elementFromPoint()` 仍命中卡片，fully-deferred 空白视角可由 Fit View 恢复。网络聚焦专项还断言选中/悬停节点不继承 dim、LOD 展开全过程没有 `visibility:hidden`、上下文卡片区域可以平移画布，且一行生产卡显示配方与产物。两份 36,704,109-byte / 11,723,913-byte 玩家档各完成 4 种卡片 × 3 种重叠、选择/悬停展开和 390×844 / 360×640 / 844×390 的 18 张截图矩阵；两轮均无 pageerror，原附件 bytes、mtime、SHA-256 未变，80/100/125/150/200% 字体继续通过。production preview 的自动档三轮没有 >50 ms 帧；Windows Chrome 在 144 Hz 显示器上会把三个刷新周期量化为 20.83 ms，因此稳定门槛为 P95 ≤21 ms，独立 max ≤100 ms 门槛不变。“完整 + 全部卡片”通过 480/1,000 统一密集保护保持可恢复；固定完整在 500 个可见、非合并重卡时仍有约 153～389 ms 帧 P95，作为明确记录的残余成本。
