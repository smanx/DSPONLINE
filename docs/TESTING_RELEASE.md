...
| 单元/领域 | `npm test` | `1.0.11` 正式版 546 项通过，3 项可选基准跳过 | 引擎、v1-v40 存档、确定性物流索引、燃料/蓄电/复杂递归制造批次、终局夹具、离线 Worker、内容包、云同步和原生更新配置等 |
| 浏览器 E2E | `npm run test:e2e` | `1.0.11` 正式版 166/166 通过 | 从开局到银河终局、物流与制造、双语亮暗主题、新旧手机壳、IndexedDB 存档保护、性能设置和横竖屏回归 |
| 云服务 | `npm run test:server` | `1.0.11` 正式版 42 项 | 用户名注册、四槽云存档、schema v3→v7、SQLite layout v1→v2、存档完整性、自动排名、内部排行榜限制、幂等处置和管理员保护 |
