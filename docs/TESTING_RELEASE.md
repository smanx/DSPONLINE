...
> **当前发布基线（2026-08-09）**：当前正式版本使用 `1.0.35 / GameState v46`；有效资产的 v43 空间站实验存档拒绝加载。量子网络回归以 GameState v46 语义为基线，并保留传统物流站升级兼容测试。1.0.35 不升级存档 envelope、云 schema 或 SQLite layout。
...
> 正式发布以源码 `080844f55852995341a5f251d8f91bf090b9403f` / Build ID `1.0.35+080844f55852` 完成两地备份、未激活目录、隔离启动、原子切换、9 文件公网下载、APK/EXE 完整哈希、Range/cache、当前/历史资源和五场 Chrome smoke。Android 使用批准长期证书；Windows 为 `NotSigned`。用户只豁免该候选的物理真机 stable 门禁，模拟器覆盖升级不能替代真机结论。生产证据见 [releases/1.0.35.md](./releases/1.0.35.md)。
...
| 单元/领域 | `npm test` | `1.0.36`：101 个文件通过、6 个跳过；918 项通过、17 项跳过 | 引擎暂停边界、递归制造守恒、科研宏观账本、普通/保守离线、纯挂机幂等与守恒、模式存档隔离、云存档兼容、v1-v46 存档、传送带/物流索引 oracle 和云同步等 |
| 浏览器 E2E | `npm run test:e2e` | `1.0.36`：259 项通过、11 项显式条件夹具跳过、0 失败 | 版本公告/历史、并联线路偏好、速通面板、快速离线、纯挂机宏观恢复、普通/速通存档隔离、Canvas 命中与回退、新旧手机壳、IndexedDB 和横竖屏回归 |
...
| 生产构建 | `npm run build` | clean `1.0.36+e0ad49062fa3` 构建通过 | `tsc -b`、Vite chunk、普通离线/宏观 Worker 和 PWA 资源；生产多 Worker 入口硬关闭 |
| 桌面目录包 | `npm run desktop:pack` | clean 目录包通过；隔离启动 10 秒；`NotSigned` | Electron 启动、包内版本与 Windows 解包 |
