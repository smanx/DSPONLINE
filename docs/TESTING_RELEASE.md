...
> `1.0.35` 未发布候选在 2026-08-09 的最终开发工作树上通过：`npm ci`（根 457 包、服务端 75 包）、typecheck、Vitest 95 文件通过/5 文件跳过且 854 项通过/16 项跳过、服务端 67 项通过/2 项跳过、运维 6/6、原生工具 8/8、128 个运行时许可证、生产构建 1,885 模块，以及 Playwright 254 项通过/11 条显式真实夹具跳过/0 失败（265 总数，705.4 秒）。根项目与服务端 `npm audit --omit=dev --audit-level=high` 均为 0；完整根审计仍有 5 个只影响开发/打包工具链的既有告警（1 moderate、4 high），不得误写成生产依赖告警。Playwright 的 `127.0.0.1:65534` API 代理拒绝是隔离用例预期噪声。
