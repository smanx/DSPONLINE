...
> **1.0.46 完整开发门禁（2026-08-18，未发布）**：普通构建默认采用 1.0.43-compatible verified-primary 协调器，durable runtime/WAL 只有显式 `VITE_DURABLE_RUNTIME_RECOVERY=true` 才启用；空间站 v46 bridge 强制稳定路径。durable 故障 E2E 7/7 覆盖 finalize、二次 persistence、T1/head rollover、运行中自动保存恢复、实验性编辑保存失败、pagehide 与撤销/视口/activity 回放。两份真实玩家档各完成两次 30 秒自动保存，均保持 Worker active、`paused=false`，重载后主档运行且源文件 SHA-256 不变。手机连续拉线覆盖重复/非法点击非阻塞、最终原子失败零创建零扣料、6/10/50/100 候选、390×844/360×640/844×390 与 80%～200% 字体；dynamic-import 分类不再吞掉普通 `TypeError`。当前 tip 已通过 typecheck；全量 Vitest 1,421/20；server 363/2 + station 3/3；ops 56/6；release-switch 29/29；native 24/24；licenses 125 包；匿名 v47 发布合同 12/12；Chromium 418/23；production preview 22/22；Firefox/WebKit 2/2；两份真实存档 autosave + canvas 4/4；根/server audit 0 漏洞。`npm run build:web` 为 1,961 modules，startup 194,838 B gzip、menu 280,997 B gzip、forbidden startup modules 0。release gate 强制 Web 平台构建并在其后运行 PWA 生命周期，禁止复用 Android/Desktop `dist`。Android/Windows 正式签名、实体设备与线上发布仍未执行。
