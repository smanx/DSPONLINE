...
| 单元/领域 | `npm test` | `0.9.0` / v33 408 项，1 项可选基准跳过 | 引擎、v1-v33 存档、物流索引等价、增产剂、无限科研、戴森复制、银河活动、生产刷新偏好、可配置缓存、离线 Worker 等价性、存档配额急救、科研、电力、蓝图、规划、网络与云同步等 |
| 浏览器 E2E | `npm run test:e2e` | `0.9.0` / v33 126 项 | 从开局到银河终局、七档生产刷新、托盘管理、球壳复制、无限科研、活动关闭态、新旧手机壳、本地存档保护和桌面/手机横竖屏回归 |
| 云服务 | `npm run test:server` | `0.9.0` 候选 27 项 | 用户名注册、旧邮箱登录、无邮件四槽云存档、schema v3→v7 迁移、设备会话、匿名统计、v33 校验、活动配置关闭/非法拒绝/确定性曲线、腾讯 SES 模板 API、排行榜邮箱门槛和管理员保护 |
...
## 18. `0.9.1` 联合空间站活动候选验收

`0.9.1` 保持 GameState v33、存档 envelope v2 和云 schema v7。候选工作树完成以下重新验证：
...
正式发布还需在最终发布 SHA 重新生成生产构建与 manifest，分别创建并验证两地 SQLite 一致性备份，在未激活目录复验服务端测试，再原子切换、统一启用活动配置并完成公网烟测。
