...
| 单元/领域 | `npm test` | 208 项 | 引擎、存档、内容、规划、网络、性能、战役等 |
| 浏览器 E2E | `npm run test:e2e` | 76 项 | 从开局到银河终局、桌面/移动交互、运营后台和匿名统计 |
| 云服务 | `npm run test:server` | 10 项 | 账号、匿名在线/访问统计、v3→v4 迁移、云存档、排行榜和管理员保护 |
...
再检查 Markdown 链接和 Skill validator。无需因文档改动重跑 76 项浏览器测试。
...
- CI 同时运行前端单元、服务端测试和浏览器关键路径；完整 76 项可作为合并或夜间门禁。
