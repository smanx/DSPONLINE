...
| 单元/领域 | `npm test` | 218 项 | 引擎、存档、行星档案、渐进教学、内容、规划、网络、性能、战役、云同步与本地账号绑定等 |
| 浏览器 E2E | `npm run test:e2e` | 82 项 | 从开局到银河终局、桌面/移动交互、渐进教学卡点定位、运营后台、账号安全、云冲突和真触摸手势 |
...
再检查 Markdown 链接和 Skill validator。无需因文档改动重跑 82 项浏览器测试。
...
- CI 同时运行前端单元、服务端测试和浏览器关键路径；完整 82 项可作为合并或夜间门禁。
