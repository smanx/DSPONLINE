...
| 浏览器 E2E | `npm run test:e2e` | 74 项 | 从开局到银河终局、桌面与移动交互 |
| 云服务 | `npm run test:server` | 5 项 | 注册、匿名玩家去重/在线过期/重启持久化、云存档修订/恢复、排行榜重算 |
...
再检查 Markdown 链接和 Skill validator。无需因文档改动重跑 74 项浏览器测试。
...
- CI 同时运行前端单元、服务端测试和浏览器关键路径；完整 74 项可作为合并或夜间门禁。
