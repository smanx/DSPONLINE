...
> **1.0.46 完整开发门禁（2026-08-19，未发布）**：固定运行时源码 `c24a0f57efeb83a54202b1ede5ce2b073820dde5`。普通构建默认采用 1.0.43-compatible verified-primary 协调器，durable runtime/WAL 只有显式 `VITE_DURABLE_RUNTIME_RECOVERY=true` 才启用；空间站 v46 bridge 强制稳定路径。durable 故障 E2E 7/7 覆盖 finalize、二次 persistence、T1/head rollover、运行中自动保存恢复、实验性编辑保存失败、pagehide 与撤销/视口/activity 回放。两份真实玩家档各完成两次 30 秒自动保存，均保持 Worker active、`paused=false`，重载后主档运行且源文件 SHA-256 不变。手机连续拉线覆盖重复/非法点击非阻塞、最终原子失败零创建零扣料、6/10/50/100 候选、390×844/360×640/844×390 与 80%～200% 字体；dynamic-import 分类不再吞掉普通 `TypeError`。画布门禁覆盖网络聚焦下交互目标不透明、LOD 展开无隐藏帧、dim 节点可平移画布、一行配方/产物标签、重叠标记、“完整 + 全部卡片”480/1,000 密集保护，以及 `onMove` 期间节点虚拟化/小地图/放置坐标同步。全量 Vitest 1,422/20；server 363/2 + station 3/3；ops 56/6；release-switch 29/29；native 24/24；licenses 125 包；匿名 v47 发布合同 12/12；Chromium 424/25；production feature 27/27、PWA 3/3、独立自动性能 3/3；Firefox/WebKit 2/2；两份真实存档各 19 张画布矩阵且 pageerror 0；根/server audit 0 漏洞。`npm run build:web` 为 1,961 modules，startup 194,813 B gzip、menu 281,356 B gzip、forbidden startup modules 0，Build ID `1.0.46+c24a0f57efeb`。release gate 强制 Web 平台构建并在其后运行 PWA 生命周期，禁止复用 Android/Desktop `dist`。未签名 Windows/Android 诊断制品已构建；正式签名、实体设备与线上发布仍未执行。
