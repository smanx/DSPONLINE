...
> **1.0.46 完整开发门禁（2026-08-18，未发布）**：固定运行时源码 `44224c862b6662023772a8410be1ea13245943e4`。普通构建默认采用 1.0.43-compatible verified-primary 协调器，durable runtime/WAL 只有显式 `VITE_DURABLE_RUNTIME_RECOVERY=true` 才启用；空间站 v46 bridge 强制稳定路径。durable 故障 E2E 7/7 覆盖 finalize、二次 persistence、T1/head rollover、运行中自动保存恢复、实验性编辑保存失败、pagehide 与撤销/视口/activity 回放。两份真实玩家档各完成两次 30 秒自动保存，均保持 Worker active、`paused=false`，重载后主档运行且源文件 SHA-256 不变。手机连续拉线覆盖重复/非法点击非阻塞、最终原子失败零创建零扣料、6/10/50/100 候选、390×844/360×640/844×390 与 80%～200% 字体；dynamic-import 分类不再吞掉普通 `TypeError`。画布展开节点以实际屏幕 hit-test 验证高于重叠 halo 和普通邻居，不再选中或悬停后消失。全量 Vitest 1,421/20；server 363/2 + station 3/3；ops 56/6；release-switch 29/29；native 24/24；licenses 125 包；匿名 v47 发布合同 12/12；Chromium 418/24；production preview 22/22；Firefox/WebKit 2/2；两份真实存档 autosave + canvas 4/4；根/server audit 0 漏洞。`npm run build:web` 为 1,961 modules，startup 194,826 B gzip、menu 281,082 B gzip、forbidden startup modules 0。release gate 强制 Web 平台构建并在其后运行 PWA 生命周期，禁止复用 Android/Desktop `dist`。未签名 Windows/Android 诊断制品已构建；正式签名、实体设备与线上发布仍未执行。

> **1.0.46 画布展示增量门禁（2026-08-18）**：一行卡及 React Flow wrapper 固定 `96×32`；数量标记为 `88×44` 点击区和约 `80×30` 可见胶囊；大幅平移后 `elementFromPoint()` 仍命中卡片，fully-deferred 空白视角可由 Fit View 恢复。两份 36,704,109-byte / 11,723,913-byte 玩家档各完成 4 种卡片 × 3 种重叠、选择/悬停展开和 390×844 / 360×640 / 844×390 的 17 张截图矩阵；两轮均无 pageerror，原附件 bytes、mtime、SHA-256 未变，80/100/125/150/200% 字体继续通过。production preview 的自动档三轮没有 >50 ms 帧；Windows Chrome 在 144 Hz 显示器上会把三个刷新周期量化为 20.83 ms，因此稳定门槛为 P95 ≤21 ms，独立 max ≤100 ms 门槛不变，连续重复 3/3 通过。固定完整和展开全部仍记录秒级长帧，不作为自动档性能结论。
