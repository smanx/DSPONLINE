...
当前公开的 Windows `1.0.5` 是明确标注的未签名测试安装包。它已通过构建、启动、更新清单和下载校验，但 Windows 仍会显示“未知发布者”或 SmartScreen 提示；取得可信代码签名证书之前不得描述为正式签名版。
...
Android `1.0.5` APK 已使用与 `1.0.0` 至 `1.0.4` 相同的长期发布密钥签名，证书 SHA-256 为 `ede2aa09ed143a3fbeb283aad0e7801d192c851f240cd39278c399918a0216ce`。APK Signature Scheme v2/v3 均通过；发布密钥路径和密码只保存在受保护环境中。
...
Android 1.0.5 的稳定清单将 `minimumSupportedVersionCode` 保持为 `1000002`：1.0.2～1.0.4 均能检测并覆盖升级到 1.0.5。1.0.2/v35 与 1.0.3～1.0.4/v36 存档继续允许上传，v37 存档禁止有损降级。受支持版本进入应用版本卡时会自动检查一次并保留手动重试。
...
- 已验证 Windows 解包版启动、`file://` 加载、FileVersion/ProductVersion 1.0.5、Stable 通道、受限 HTTPS API 和更新基址。
- 已验证 Android API 36.1 模拟器从已安装的正式签名 `1.0.4` 使用 `adb install -r` 原地升级到 `1.0.5`；`firstInstallTime` 保持不变，应用启动后最近日志无致命异常。卸载应用仍会删除本机应用数据。
- Android 稳定 APK 为 `1.0.5` / `1000005`，大小 4,267,825 字节，SHA-256 为 `1d9b918e621b187e9f97d6f01c822d4b35943a112fda9ff420985d70cfd9fb7c`。APK v2/v3 和批准证书均通过；AAB 未进入下载站或应用商店。
- Windows x64 安装程序版本为 `1.0.5`，大小 112,039,520 字节，SHA-256 为 `12ca09de705a72830c7a224bcc8a756ae43f5ba6532cfc8eec4a85e615dc3220`。Authenticode 状态为 `NotSigned`，下载页会持续显示未知发布者警告。
- 上海下载站当前目录为 `/var/www/dsp-idle-downloads/releases/1.0.5-af8593bc5de4`，上一目录为 `1.0.4-9ca10b44507a`；旧版目录与安装包继续保留。二进制使用 immutable 缓存，更新清单使用 no-cache，Range 请求返回 `206`，香港 `/downloads/*` 重定向至上海下载域名。
- 1.0.5 原生制品来自干净 Git 提交 `af8593bc5de48a88ad11cf5ee0264f9ddc1cef28`，包内版本、官方 API、更新源与公网完整下载哈希均已复验。
