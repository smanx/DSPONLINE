...
当前公开的 Windows `1.0.33` 是明确标注的未签名测试安装包。它已通过构建、隔离启动、更新清单和下载校验，但 Windows 仍会显示“未知发布者”或 SmartScreen 提示；取得可信代码签名证书之前不得描述为正式签名版。
...
Android 1.0.33 的稳定清单将 `minimumSupportedVersionCode` 保持为 `1000002`：1.0.2～1.0.32 均能检测并覆盖升级到 1.0.33。服务端继续接受合法 v35-v46 存档，客户端不允许把 v46 有损降级。受支持版本进入应用版本卡时会自动检查一次并保留手动重试。
...
- Web/API 与双原生制品来自发布标识 `1.0.33-2bd81de8d7f1`，包内版本、Build ID、官方 API、更新源与公网完整文件哈希均已复验。完整生产备份、切换和残余风险见 [releases/1.0.33.md](./releases/1.0.33.md)。
