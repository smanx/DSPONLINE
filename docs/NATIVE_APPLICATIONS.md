...
> 当前公开稳定测试版本：Windows `1.0.9`；Android `1.0.9 / 1000009`
> Windows 包名：`com.dspidle.network`
> Android applicationId：`cn.dsponline.network`
> Android 与 Windows 1.0.9 均与 Web 共用 `GameState` v40。两端存档 envelope v2 和云 schema v7 不变；v1-v39 存档通过连续守恒迁移载入。
...
当前公开的 Windows `1.0.9` 是明确标注的未签名测试安装包。它已通过构建、启动、更新清单和下载校验，但 Windows 仍会显示“未知发布者”或 SmartScreen 提示；取得可信代码签名证书之前不得描述为正式签名版。
...
Android `1.0.9` APK 已使用与 `1.0.0` 至 `1.0.8` 相同的长期发布密钥签名，证书 SHA-256 为 `ede2aa09ed143a3fbeb283aad0e7801d192c851f240cd39278c399918a0216ce`。APK Signature Scheme v2/v3 均通过；发布密钥路径和密码只保存在受保护环境中。
...
Android 1.0.9 的稳定清单将 `minimumSupportedVersionCode` 保持为 `1000002`：1.0.2～1.0.8 均能检测并覆盖升级到 1.0.9。服务端继续接受合法 v35-v40 存档，客户端不允许把 v40 有损降级。受支持版本进入应用版本卡时会自动检查一次并保留手动重试。
...
- 已验证 Windows 解包版隔离启动、`file://` 加载、FileVersion/ProductVersion 1.0.9、Stable 通道、受限 HTTPS API 和更新基址。
- 已验证 Android API 36.1 模拟器从已安装的正式签名 `1.0.8` 使用 `adb install -r` 原地升级到 `1.0.9`；`firstInstallTime` 保持不变、旧版本创建的本地工厂可继续载入，启动后日志无 Fatal 或 ANR。卸载应用仍会删除本机应用数据。
- Android 稳定 APK 为 `1.0.9` / `1000009`，大小 4,302,846 字节，SHA-256 为 `3b22118ebd8dd3dd4544063b953c8bfd0c48781cbd6765ac546274e61723863b`。APK v2/v3 和批准证书均通过；AAB 未进入下载站或应用商店。
- Windows x64 安装程序版本为 `1.0.9`，大小 103,077,273 字节，SHA-256 为 `b1f2c6b97689610cd7ee23d44abfe997c6330cb2f265071878b74a2ca7a28deb`。Authenticode 状态为 `NotSigned`，下载页会持续显示未知发布者警告。
- 上海下载站当前目录为 `/var/www/dsp-idle-downloads/releases/1.0.9-8a69426b5c9b`，下载回滚目录为 `1.0.8-528455cdfc2b`；旧版目录与安装包继续保留。二进制使用 immutable 缓存，更新清单使用 no-cache，Range 请求返回 `206`，香港 `/downloads/*` 重定向至上海下载域名。
- 1.0.9 原生制品来自干净 Git 提交 `8a69426b5c9ba7dc4be43170059000e192538715`，包内版本、官方 API、更新源与公网文件哈希均已复验。
