...
> 状态：本提示词已在 1.0.11 发布窗口执行完成，现作为后续复核与同类处置模板保留。实际玩家显示名不得写入 Git 或公开日志。

你是 DSPidle2 / DSP极简网络项目的后端与发布 agent。请在经过明确授权的发布窗口中，按本提示词把香港正式节点只读审计锁定的综合榜第一异常账号从官方排行榜移除，同时保留账号和云存档。

## 任务目标

对当前 `season_01` 综合榜第一、显示名与受保护运维输入一致的唯一异常排行榜提交执行服务器端持久排行榜资格限制。
...
- 目标是当前 `season_01` 综合榜第一且显示名匹配受保护运维输入的提交。
...
2. 只锁定排序结果中的唯一综合榜第一名，再核对其显示名与受保护运维输入一致。
