...
- P2 画布控件遮挡：最终 Chromium 复核发现节点拖动后，选区工具栏会在 `1280x820` 下覆盖 React Flow 的“适应视图”控件并拦截点击，同时压住小地图底边和运行记录。现在选区打开时会为缩放控件、小地图、折叠按钮和运行记录预留互不重叠的空间；原失败旅程连续 3/3，手机竖屏/横屏选区旅程 2/2 通过。
- P2 E2E 并发隔离：全量压力 trace 证明 tooltip 本身已经出现，但一次迟到的 1 秒离线结算报告拦截鼠标，而无单次超时的 `hover()` 独占了外层 15 秒重试预算。UI 套件现在与其他 game-flow 套件一致地自动确认非目标离线报告，并限制单次 hover 等待；研究拖动、手动冶炼和 tooltip 在 4-worker 下重复 12/12，移动选区与字体/画布矩阵 4/4 通过，产品断言和总超时均未放宽。
