...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY.map((entry) => Number(entry.version.split(".")[2]))).toEqual([30, 29, 28, 27, ...Array.from({ length: 25 }, (_, index) => 24 - index)]);
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 2).map((entry) => entry.version)).toEqual(["1.0.30", "1.0.29"]);
    expect(getReleaseNotesPage(12, 2).map((entry) => entry.version)).toEqual(["1.0.4", "1.0.3"]);
  });
});
