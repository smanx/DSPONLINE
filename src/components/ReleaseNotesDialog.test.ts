...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY.map((entry) => Number(entry.version.split(".")[2]))).toEqual(Array.from({ length: 36 }, (_, index) => 35 - index));
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 2).map((entry) => entry.version)).toEqual(["1.0.35", "1.0.34"]);
    expect(getReleaseNotesPage(15, 2).map((entry) => entry.version)).toEqual(["1.0.5", "1.0.4"]);
    expect(getReleaseNotesPage(16, 2).map((entry) => entry.version)).toEqual(["1.0.3", "1.0.2"]);
    expect(getReleaseNotesPage(17, 2).map((entry) => entry.version)).toEqual(["1.0.1", "1.0.0"]);
  });

  it("maps direct page jumps and historical details to the same page", () => {
    expect(getReleaseNotesPageCount()).toBe(12);
    expect(getReleaseNotesPageForRelease("2026-08-09-v1.0.35")).toBe(0);
    expect(getReleaseNotesPageForRelease("2026-08-07-v1.0.33")).toBe(0);
    expect(getReleaseNotesPageForRelease("2026-08-07-v1.0.32")).toBe(1);
    expect(getReleaseNotesPageForRelease("2026-08-06-v1.0.31")).toBe(1);
    expect(getReleaseNotesPageForRelease("2026-08-03-v1.0.24")).toBe(3);
