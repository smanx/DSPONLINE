...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY.map((entry) => Number(entry.version.split(".")[2]))).toEqual(Array.from({ length: 46 }, (_, index) => 45 - index));
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 2).map((entry) => entry.version)).toEqual(["1.0.45", "1.0.44"]);
    expect(getReleaseNotesPage(18, 2).map((entry) => entry.version)).toEqual(["1.0.9", "1.0.8"]);
    expect(getReleaseNotesPage(19, 2).map((entry) => entry.version)).toEqual(["1.0.7", "1.0.6"]);
    expect(getReleaseNotesPage(20, 2).map((entry) => entry.version)).toEqual(["1.0.5", "1.0.4"]);
    expect(getReleaseNotesPage(21, 2).map((entry) => entry.version)).toEqual(["1.0.3", "1.0.2"]);
  });
  ...
  it("serves the current release from stable locale keys", () => {
    ...
    expect(chinese).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.0.45" });
    expect(english).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.0.45" });
