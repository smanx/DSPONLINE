...
const SECTIONS: TutorialSection[] = [
  ...
  { id: "time-warp", title: "时间扭曲纯挂机", goal: "在不操作工厂的情况下提高真实模拟倍率。", prerequisites: "放置时间扭曲装置并接入足够电力。", steps: ["选中装置并设为主控，选择请求倍率。", "点击“开始纯挂机”，进入独立挂机页面；画布、建造和选择会被冻结。", "挂机页只显示实际倍率、挂机时间、模拟积压、关键产量和保存状态。", "点击“停止挂机”后等待剩余计算和存档校验，再返回工厂。"], success: "停止后返回同一工厂，暂停期间没有补算，进度从最后有效快照恢复。", mistakes: ["把挂机页面关闭当成停止。", "供电不足却期待请求倍率完全生效。"], recovery: "刷新会恢复最后有效存档；重新进入后查看实际倍率和降档原因，确认后再开始。" },
